...
**Purpose:** Summarize all articles for a single client, categorize by project, and generate per-project narratives.
...
sources are reporting about a specific client and their projects.
...
1. Articles from the client's own media pages (press releases, news
   ...
   correct project based on the project definitions provided
...
Write a briefing for this client covering the past week's activity.

1. CATEGORIZE each article into the most relevant project from the
   project definitions above. If an article doesn't fit any defined
   project, categorize it as "General".

2. For EACH project that has articles, write a 100-200 word summary that:
   - States what was reported (factual)
   - Notes the source and whether it's from the client's own media or
     an industry publication (industry mentions are often more significant)
   - Highlights why the account manager should care

3. For "General" articles, write a brief 50-100 word note.
...
RETURN THIS EXACT JSON STRUCTURE:
{
  ...
  "general_mentions": {
    "summary": "<50-100 word summary or null if none>",
    "mention_count": <number>,
    "articles": [...]
  },
...
**Purpose:** Select the 3–5 most significant findings across all clients.
...
4. Industry coverage (mentions in major publications carry extra weight)
5. Emerging risks or opportunities

Industry publication mentions often carry more weight than client self-
published content, since third-party coverage suggests broader significance.
```
...
|-------------|-----------|-------|
| **0.3** | Consistent, structured selection | Highlight Selector |
| **0.4** | Factual and concise with natural writing | Per-Client Summarizer |
